    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Text("Live delay controls \u{2014} a slider plus \u{201C}Delay 1 Second\u{201D} and \u{201C}Skip 1 Second\u{201D} buttons \u{2014} are on the Now Playing page of the AntennaHead tab.\n\nAdds a \u{201C}PCMDelay\u{201D} stage that holds the audio back by up to \(Int(SDRController.maxAudioDelaySeconds) / 60) minutes, so a radio play-by-play can be lined up with a TV picture that runs behind it. A short chirp is mixed into the audio each time a change to the delay takes effect, so you can tell when the last adjustment has landed. The stage sits ahead of the speech-to-text and spatial stages, so captions stay in step with the delayed audio. Turning this switch on or off applies from the next tuning. Each new tuning starts with the current delay as silence: the spoken \u{201C}now playing\u{201D} announcement plays at the start of that silence (and is skipped if the delay is too short to fit it), and with the countdown on, beeps every second and a spoken countdown (every 30 seconds above a minute, every 5 seconds from a minute down to 15, then every second for the last 10) lead up to the delayed audio. Memory is committed only as the delay fills \u{2014} about 11 MB per minute of delay actually buffered, up to roughly 115 MB at 10 minutes. Control messages go to UDP port \(Int(sdrController.audioDelayControlPort)) on this Mac.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
